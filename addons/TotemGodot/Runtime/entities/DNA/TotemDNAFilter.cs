...
namespace TotemServices.DNA
{
	...
	public class TotemDNAFilter
	{
		...
		{ 
			get
			{
				using var file = FileAccess.Open("res://addons//TotemGodot//Resources//" + ServicesEnv.SmartContractAvatarsFilterName, FileAccess.ModeFlags.Read);
				string content = file.GetAsText();
				
				//GD.Print(content);
				return new TotemDNAFilter(content);
			}
		}
		...
		{ 
			get
			{
				using var filterAsset = FileAccess.Open("res://addons//TotemGodot//Resources//" + ServicesEnv.SmartContractItemsFilterName, FileAccess.ModeFlags.Read);
				return new TotemDNAFilter(filterAsset.GetAsText());
			}
		}
			...
			foreach (var rule in rules)
			{
				...
				switch (dnaType)
				{
					...
					case TotemDNAType.Color:
						...
						Color color32 = new Color(r/255, g/255, b/255, 1);
			...
			foreach (var rule in rules)
			{
				...
				switch (dnaType)
				{
					...
					case TotemDNAType.Color:
						...
						byte b = Convert.ToByte(binVal.Substring(16, 8), 2);
						//Color32 color32 = new Color32(r, g, b, 255);
						Color color32 = new Color(r / 255, g / 255, b / 255, 255 / 255);
