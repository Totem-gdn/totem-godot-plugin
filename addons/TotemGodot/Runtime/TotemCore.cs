...
public partial class TotemNode : Node
{
	...
	public TotemNode(TotemLegacyService legacyService, TotemAuth auth, TotemSmartContractManager smartContract, TotemAnalytics analytics, TotemDebug debug)
	{
	...
	{
		base._EnterTree();
		AddChild(_legacyService);
		AddChild(_auth);
		AddChild(_smartContract);
		AddChild(_analytics);
		AddChild(_debug);
	}
...
public class TotemCore
{
	...
	{
		//GD.Print("AuthenticateCurrentUser");
		_auth.LoginUser((user) =>
		{
			//GD.Print("onComplete LoginUser");
			if (user != null)
			{
				CurrentUser = user;
	...
	public void GetLegacyRecords(object asset, TotemAssetType assetType, UnityAction<List<TotemLegacyRecord>> onSuccess, string gameId = "")
	{
		...
		if (assetId < 0)
		{
			GD.PrintErr("Asset ID was not found!");
			return;
		}
	...
	public void AddLegacyRecord(object asset, TotemAssetType assetType, string data, UnityAction<TotemLegacyRecord> onSuccess = null)
	{
		...
		{
			GD.Print($"Legacy record created");
	...
	private void CreateServicesGameObject()
	{
		...
		*/
		_legacyService = new TotemLegacyService();
		_analytics = new TotemAnalytics();
		_auth = new TotemAuth();
		_smartContract = new TotemSmartContractManager();
		_debug = new TotemDebug();


		TotemNode = new TotemNode(_legacyService, _auth, _smartContract, _analytics, _debug);
